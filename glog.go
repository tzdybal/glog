...
// Modifications copyright 2017 Tomasz Zdybał. All Rights Reserved.
//
...
//  - if MaxSize or RotaitonInterval is reached, current file became old file,
//    and new file is created as a current log file
...
func (sb *syncBuffer) shouldRotate(len int, now time.Time) bool {
	if sb.nbytes+uint64(len) >= MaxSize {
		return true
	}
...
func gzipFile(name string) error {
	...
	// remove the plain text log only if gzipped file is synced to disk
